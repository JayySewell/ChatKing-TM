...
export const antiScanMiddleware = (
  req: SecurityRequest,
  res: Response,
  next: NextFunction,
) => {
  ...
  const isScanTool = suspiciousAgents.some((tool) =>
...
export const headerSecurityMiddleware = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const geoBlockMiddleware = async (
  ...
  try {
    ...
    const suspiciousRanges = [
      "10.",
      "172.16.",
      "192.168.", // Private ranges (shouldn't hit public server)
      "169.254.", // Link-local
    ];

    const isSuspiciousRange = suspiciousRanges.some((range) =>
      ip.startsWith(range),
    );
