...
export class FeedbackService {
  async submitFeedback(
    feedbackData: Omit<
      FeedbackEntry,
      "id" | "createdAt" | "updatedAt" | "status" | "votes"
    >,
  ): Promise<string> {
    try {
      ...
      // Update user stats
      await enhancedAuthService.updateStats(
        feedbackData.userId,
        "feedback" as any,
      );
    ...
    try {
      ...
      if (filters.status) {
        filteredFeedback = filteredFeedback.filter(
          (f) => f.status === filters.status,
        );
      }
      if (filters.type) {
        filteredFeedback = filteredFeedback.filter(
          (f) => f.type === filters.type,
        );
      }
      if (filters.category) {
        filteredFeedback = filteredFeedback.filter(
          (f) => f.category === filters.category,
        );
      }
      if (filters.severity) {
        filteredFeedback = filteredFeedback.filter(
          (f) => f.severity === filters.severity,
        );
      }
    ...
    try {
      ...
      filteredFeedback.forEach((feedback) => {
        ...
        stats.bySeverity[feedback.severity] =
          (stats.bySeverity[feedback.severity] || 0) + 1;
        stats.byStatus[feedback.status] =
          (stats.byStatus[feedback.status] || 0) + 1;
      });

      // Calculate average rating
      const ratedFeedback = filteredFeedback.filter(
      ...
      if (resolvedFeedback.length > 0) {
        ...
        stats.resolutionTime.average =
          resolutionTimes.reduce((a, b) => a + b, 0) / resolutionTimes.length;
        stats.resolutionTime.median = resolutionTimes.sort((a, b) => a - b)[
      ...
      const satisfactionFeedback = filteredFeedback.filter(
        (f) => f.type === "compliment" || (f.rating && f.rating >= 4),
      );
      stats.userSatisfaction =
        filteredFeedback.length > 0
          ? (satisfactionFeedback.length / filteredFeedback.length) * 100
          : 0;
  ...
  private async notifyAdminsOfCriticalFeedback(
