...
export class ConfigValidator {
  ...
  validateConfiguration(): {
    ...
    const requiredConfigs = [
      {
        ...
        required: true,
        validator: (value: string) => value.startsWith('sk-or-v1-') && value.length > 20,
      },
      {
        service: 'Pinecone',
        key: 'PINECONE_API_KEY',
        required: true,
        validator: (value: string) => (value.startsWith('pcsk_') || value.startsWith('pc-')) && value.length > 20,
      },
      {
        service: 'BraveSearch',
        key: 'BRAVE_SEARCH_API_KEY',
        required: false,
        validator: (value: string) => value.startsWith('BSA') && value.length > 10,
      },
      {
        service: 'Google OAuth',
        key: 'GOOGLE_CLIENT_ID',
        required: false,
        validator: (value: string) => value.length > 10 && value.includes('.googleusercontent.com') === false,
      },
      {
      ...
      },
      {
        ...
        required: false,
        validator: (value: string) => value.startsWith('SG.') && value.length > 20,
      },
      {
        service: 'JWT Secret',
        key: 'JWT_SECRET',
    ...
    for (const config of requiredConfigs) {
      ...
      const validation: ConfigValidation = {
        ...
        present,
        valid: present && valid,
        message: this.getValidationMessage(config.service, present, valid, config.required),
      };
   ...
   */
  getPublicConfigStatus(): Record<string, { 
    configured: boolean; 
    valid: boolean; 
    service: string; 
  }> {
    const validation = this.validateConfiguration();
    const publicStatus: Record<string, { configured: boolean; valid: boolean; service: string }> = {};
  ...
  getEnvironmentStatus(): {
    ...
    const configuredCount = validation.validations.filter(v => v.present).length;
  ...
  generateSecureConfig(): {
    ...
    for (const v of validation.validations) {
      ...
      if (value) {
        ...
        if (value.includes('demo') || value.includes('test') || value.includes('dev')) {
    ...
    if (!process.env.JWT_SECRET || process.env.JWT_SECRET.length < 64) {
      recommendations.push('Use a longer JWT secret (64+ characters) for better security');
      securityScore -= 10;
    }

    if (!process.env.ENCRYPTION_KEY || process.env.ENCRYPTION_KEY.length < 64) {
      recommendations.push('Use a longer encryption key (64+ characters) for better security');
      securityScore -= 10;
    }
  ...
  logConfigurationStatus(): void {
    ...
    console.log(`   Configured: ${status.configuredCount}/${status.totalConfigs}`);
