...
export class OAuthService {
  private googleClientId = process.env.GOOGLE_CLIENT_ID || 'demo-google-client-id';
  private googleClientSecret = process.env.GOOGLE_CLIENT_SECRET || 'demo-google-secret';
  ...
  async handleAppleCallback(code: string, state: string, user?: string): Promise<any> {
    try {
      // Exchange code for tokens
      ...
      return await this.createOrLoginUser({
        ...
        name: userData.name ? `${userData.name.firstName} ${userData.name.lastName}` : userData.email.split('@')[0],
  ...
  private createMockAppleIdToken(): string {
    ...
    const signature = crypto.createHmac('sha256', 'demo-secret').update(`${header}.${body}`).digest('base64');
  ...
  private async createOrLoginUser(oAuthData: {
    ...
    try {
      ...
      if (user) {
        ...
        const isOwner = userCount === 0 || oAuthData.email === 'jayysewell18@gmail.com';
