...
export const handleGetThreatIntelligence: RequestHandler = async (req, res) => {
  try {
    ...
    if (stats.totalEvents > 1000) {
      threatAnalysis.recommendations.push(
        "Consider implementing stricter rate limiting",
      );
    }
    if (stats.blockedIps > 50) {
      threatAnalysis.recommendations.push(
        "High number of blocked IPs detected - review security rules",
      );
    }
    if (
      stats.topThreats.some((t) => t.type === "ddos_attempt" && t.count > 100)
    ) {
      threatAnalysis.recommendations.push(
        "DDoS protection may need enhancement",
      );
    }
...
router.post(
  ...
  authMiddleware,
  ownerOnlyMiddleware,
  handleTestSecurityAnalysis,
);
router.get(
  "/threat-intelligence",
  authMiddleware,
  ownerOnlyMiddleware,
  handleGetThreatIntelligence,
);
