...
export const handleClearCalculatorHistory: RequestHandler = async (req, res) => {
  try {
...
export const handleGetCalculatorConstants: RequestHandler = async (req, res) => {
  try {
    const constants = {
      mathematical: {
        pi: { value: Math.PI, description: 'Pi (π)', symbol: 'π' },
        e: { value: Math.E, description: 'Euler\'s number (e)', symbol: 'e' },
        phi: { value: (1 + Math.sqrt(5)) / 2, description: 'Golden ratio (φ)', symbol: 'φ' },
        sqrt2: { value: Math.sqrt(2), description: 'Square root of 2', symbol: '√2' },
        sqrt3: { value: Math.sqrt(3), description: 'Square root of 3', symbol: '√3' },
      },
      physical: {
        c: { value: 299792458, description: 'Speed of light (m/s)', symbol: 'c' },
        g: { value: 9.80665, description: 'Standard gravity (m/s²)', symbol: 'g' },
        h: { value: 6.62607015e-34, description: 'Planck constant (J⋅s)', symbol: 'h' },
        k: { value: 1.380649e-23, description: 'Boltzmann constant (J/K)', symbol: 'k' },
        Na: { value: 6.02214076e23, description: 'Avogadro number (mol⁻¹)', symbol: 'Na' },
      }
    };
