...
export class CKStorage {
  ...
  constructor(basePath: string = "./ck-data") {
    ...
    this.encryptionKey =
      process.env.CK_ENCRYPTION_KEY || "default-key-change-in-production";
  ...
  async getUserByEmail(email: string): Promise<UserData | null> {
    try {
      ...
      for (const file of files) {
        const user = await this.readFile<UserData>(join(usersDir, file), {
          encrypt: true,
        });
  ...
  async getUserChatSessions(userId: string): Promise<ChatSession[]> {
    try {
      ...
      for (const file of files) {
        const session = await this.readFile<ChatSession>(join(chatsDir, file), {
          encrypt: true,
        });
  ...
  async getUserSearchHistory(
    ...
    try {
      ...
      for (const file of files) {
        const search = await this.readFile<SearchHistory>(
          join(searchDir, file),
          { encrypt: true },
        );
  ...
  async getUserCalculatorLogs(
    ...
    try {
      ...
      for (const file of files) {
        const log = await this.readFile<CalculatorLog>(join(calcDir, file), {
          encrypt: true,
        });
  ...
  async getUserPineconeIndexes(userId: string): Promise<PineconeIndex[]> {
    try {
      ...
      for (const file of files) {
        const index = await this.readFile<PineconeIndex>(
          join(pineconeDir, file),
          { encrypt: true },
        );
        ...
        try {
          ...
          switch (dir) {
            ...
            case "chats":
              ...
              break;
            case "search":
              stats.totalSearches = files.length;
              break;
            case "calculator":
              stats.totalCalculations = files.length;
              break;
            case "pinecone":
              stats.totalIndexes = files.length;
              break;
          }
        } catch (error) {
