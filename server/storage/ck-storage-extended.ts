...
export class CKStorageExtended {
  ...
  async getLoginAttempts(
    ...
    return allAttempts.filter(
      (attempt) =>
        attempt.ip === ip && new Date(attempt.timestamp).getTime() > cutoff,
    );
  }
  ...
  async storeEnvironmentVariables(
  ...
  async storePineconeVector(
    ...
    await this.store(
      `users/${userId}/pinecone-vectors/${indexName}`,
      vectorId,
      {
        ...vector,
