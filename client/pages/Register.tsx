...
import { Eye, EyeOff, Zap, Crown, Shield, User, Mail, Lock, AlertTriangle, CheckCircle } from 'lucide-react';
...
export default function Register() {
  ...
  const passwordRequirements = [
    ...
    { met: password === confirmPassword && password !== '', text: 'Passwords match' },
    ...
    <div className="min-h-screen bg-main-bg flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-text-muted text-sm sm:text-base">Create your AI platform account</p>
        </div>
          ...
          <form onSubmit={handleRegister} className="space-y-6">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-main-bg text-text-muted">Or register with email</span>
              </div>
            </div>
            ...
            {password && (
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-text-primary">Password Requirements:</h4>
                <div className="grid grid-cols-2 gap-2">
                  {passwordRequirements.map((req, index) => (
                    <div key={index} className="flex items-center space-x-2">
                      <CheckCircle className={`w-3 h-3 ${req.met ? 'text-neon-green' : 'text-text-muted'}`} />
                      <span className={`text-xs ${req.met ? 'text-neon-green' : 'text-text-muted'}`}>
                        {req.text}
                      </span>
                    </div>
                  ))}
            ...
            <div className="flex items-start space-x-3">
              ...
              <label htmlFor="terms" className="text-sm text-text-muted">
                I agree to the{' '}
                <Link to="/terms" className="text-cyber-blue hover:text-cyber-blue-light underline">
                  Terms of Service
                </Link>{' '}
                and{' '}
                <Link to="/privacy" className="text-cyber-blue hover:text-cyber-blue-light underline">
                  Privacy Policy
                </Link>
              </label>
            </div>
