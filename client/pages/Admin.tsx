...
import { 
  Crown, Users, Database, Activity, Settings, Key, 
  BarChart3, Shield, Zap, Globe, Brain, Calculator,
  Server, HardDrive, Cpu, Wifi, AlertTriangle
} from 'lucide-react';
...
export default function Admin() {
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'analytics' | 'settings' | 'api-keys'>('overview');
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'inactive': return 'text-neon-amber';
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="glass-card">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-text-muted text-sm">AI Conversations</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="glass-card">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between p-3 bg-secondary-bg rounded">
                      ...
                      <span className="text-neon-green">{formatBytes(systemStats?.storageSize || 0)}</span>
                    </div>
                ...
                <div className="glass-card">
                  ...
                  <div className="space-y-4">
                    {apiKeys.map((api) => (
                      <div key={api.name} className="flex items-center justify-between p-3 bg-secondary-bg rounded">
                        <div className="flex items-center space-x-2">
                          <Zap className="w-5 h-5 text-cyber-blue" />
                          <span className="text-text-primary">{api.name}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <div className={`w-2 h-2 rounded-full bg-current ${getStatusColor(api.status)}`}></div>
          ...
          {activeTab === 'api-keys' && (
            <div className="space-y-6">
              <div className="glass-card">
                ...
                <div className="space-y-6">
                  {apiKeys.map((api) => (
                    <div key={api.name} className="border border-border-glow rounded-lg p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h4 className="font-medium text-text-primary text-lg">{api.name}</h4>
                          <p className="text-text-muted text-sm">
                            Last used: {api.lastUsed ? new Date(api.lastUsed).toLocaleDateString() : 'Never'}
                          </p>
                        </div>
                        <div className={`px-3 py-1 rounded text-sm ${getStatusColor(api.status)} bg-current bg-opacity-10`}>
                          {api.status}
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex space-x-2">
                          <input
                            ...
                            readOnly
                          />
                          <button className="btn-cyber px-4">
                            Update
                          </button>
                        </div>
                      </div>
                ...
                <div className="mt-6 p-4 bg-neon-amber/10 border border-neon-amber/30 rounded">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-sm text-neon-amber">
                      ...
                      <p>API keys are encrypted and stored securely in the CK-Storage system. Only you have access to these keys.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="glass-card">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div className="text-center p-6 bg-secondary-bg rounded-lg">
                    ...
                    <div className="text-sm text-text-muted">AI Conversations</div>
                  </div>
                  ...
                  <div className="text-center p-6 bg-secondary-bg rounded-lg">
                    ...
                    <div className="text-sm text-text-muted">Vector Indexes</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              <div className="glass-card">
                ...
                <div className="space-y-6">
                  <div>
                    <h4 className="font-medium text-text-primary mb-3">Security Settings</h4>
                    <div className="space-y-3">
                      <label className="flex items-center space-x-3">
                        <input type="checkbox" className="rounded" defaultChecked />
                        <span className="text-text-primary">Enable encryption for all data</span>
                      </label>
                      <label className="flex items-center space-x-3">
                        <input type="checkbox" className="rounded" defaultChecked />
                        <span className="text-text-primary">Require authentication for API access</span>
                      </label>
                      <label className="flex items-center space-x-3">
                        <input type="checkbox" className="rounded" defaultChecked />
                        <span className="text-text-primary">Enable audit logging</span>
                      </label>
                    </div>
                  </div>

                  <div>
                    <h4 className="font-medium text-text-primary mb-3">Storage Settings</h4>
                    <div className="space-y-3">
                      <div>
                  ...
                  <div>
                    <h4 className="font-medium text-text-primary mb-3">System Limits</h4>
            ...
            <div className="space-y-6">
              <div className="glass-card">
                ...
                <div className="text-center py-12">
                  ...
                  <h4 className="text-xl font-medium text-text-primary mb-2">User Management</h4>
                  <p className="text-text-muted mb-6">
                    Manage user accounts, permissions, and access levels.
                  </p>
                  <p className="text-cyber-blue">
                    Continue prompting to have this section built out with full user management!
                  </p>
                </div>
              </div>
            </div>
